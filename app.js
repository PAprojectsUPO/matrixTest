...
let currentSubject = null; // 'algebra' or 'calculo'

// Quiz configuration
const MAX_QUESTIONS = 10; // Maximum number of questions per quiz

// Modules files by subject
const SUBJECT_MODULES = {
    algebra: './data/algebra/modules.json',
    calculo: './data/calculo/modules.json'
};
...
let subjectSelectionScreen;
let selectAlgebraButton;
let selectCalculoButton;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    selectAlgebraButton = document.getElementById('select-algebra');
    selectCalculoButton = document.getElementById('select-calculo');
    backToSubjectsButton = document.getElementById('back-to-subjects');
    ...
    if (selectAlgebraButton) selectAlgebraButton.addEventListener('click', () => selectSubject('algebra'));
    if (selectCalculoButton) selectCalculoButton.addEventListener('click', () => selectSubject('calculo'));
    if (backToSubjectsButton) backToSubjectsButton.addEventListener('click', showSubjectSelectionScreen);
    ...
    // Set initial screen visibility: Subject selection should be visible first.
    showSubjectSelectionScreen();
});

function showSubjectSelectionScreen() {
    if (subjectSelectionScreen) subjectSelectionScreen.classList.remove('hidden');
    ...
    if (appHeader) appHeader.textContent = 'Test de Matemáticas';
}

async function selectSubject(subject) {
    currentSubject = subject;

    // Update header with subject name
    const appHeader = document.getElementById('app-header');
    if (appHeader) appHeader.textContent = `Test de ${SUBJECT_NAMES[subject]}`;

    // Fetch modules for selected subject
    await fetchQuestions();
}

async function fetchQuestions() {
    try {
        // Use the modules file for the selected subject (default to algebra)
        const modulesFile = SUBJECT_MODULES[currentSubject] || SUBJECT_MODULES.algebra;
        ...
        // This is now just the metadata list
        const modulesMetadata = await response.json();

        if (!Array.isArray(modulesMetadata)) {
            throw new Error("Invalid data structure: modules list is not an array");
        }

        allModulesData = modulesMetadata;

        // Show module selection screen, hide subject selection
        if (subjectSelectionScreen) subjectSelectionScreen.classList.add('hidden');
...
async function selectModule(moduleMetadata) {
    selectedModuleId = moduleMetadata.id;

    // Show loading state implies UI feedback, here we just await.
    // Ideally we'd show a spinner. 
    // Let's change the header or something temporarily or just wait (fast local).

    try {
        const response = await fetch(moduleMetadata.file);
        if (!response.ok) {
            throw new Error(`Failed to load module file: ${response.status}`);
        }
        const moduleData = await response.json();

        selectedModuleData = moduleData; // Store complete module data (questions, etc)
        allQuestionsData = moduleData.difficultyLevels; // Load questions for the selected module

        // Update module info display
        if (selectedModuleName) selectedModuleName.textContent = moduleData.name;
        if (selectedModuleDescription) selectedModuleDescription.textContent = moduleData.description;

        showDifficultyScreen();

    } catch (err) {
        console.error("Error loading module details:", err);
        alert("Error al cargar el contenido del módulo. Por favor intenta de nuevo.");
    }
}
...
function displayQuestion() {
    ...
    if (window.MathJax) {
        const elementsToTypeset = [questionText, questionHintElem, optionsContainer].filter(Boolean);
